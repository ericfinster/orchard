...
trait ModuleEntries { thisInterpreter : Interpreter =>
  ...
  // sealed trait ExpressionEntry extends ModuleEntry { def expression : CellExpression }
  ...
  //         succeed(ModuleZipper(seal(node, left ++ Vector(focus) ++ right), cs))
  //     }
  ...
  //         } else {
  //           succeed(ModuleZipper(left.last, ModuleContext(node, left.init, focus +: right) :: cs))
  //         }
  //       }
  //     }
  ...
  //           succeed(ModuleZipper(rightPlus.head, ModuleContext(node, left, rightPlus.tail) :: context))
  //         }
  ...
  //       case ModuleContext(node, left, right) :: cs =>
  //         succeed(ModuleZipper(entry, ModuleContext(node, left :+ focus, right) :: cs))
  ...
  //     focus match {
  //       case m : Module => {
  //         succeed(ModuleZipper(entry, ModuleContext(m.moduleNode, m.entries, Vector.empty) :: context))
  //       }
  //       case _ => fail("Focused entry is not a module")
  //     }
